...
import {
  anonymousDraftLifetimeMs,
  isAnonymousDraftExpired,
} from "./reporting_lifecycle";
...
const requireGateway = (provided: string): void => {
  if (provided !== env.EFFI_GATEWAY_CONVEX_SECRET)
...
const effiTranscriptSource = v.union(
  v.literal("assistant_message"),
  v.literal("input_request"),
);
...
const normalizedTranscript = (
  ...
  acceptedEvidenceById: ReadonlyMap<
    string,
    Infer<typeof acceptedEvidenceValidator>
  >,
) =>
  messages.map((message) => {
    if (message.direction === "citizen") {
      if (!("id" in message.payload))
        throw new Error("A citizen transcript entry has invalid content.");
      const payload = message.payload;
      const attachments = (payload.attachments ?? []).map((attachment) => {
        if (!attachment.storageKey || !hasText(attachment.mediaType)) {
          throw new Error(
            "A transcript attachment is missing durable metadata.",
          );
        }
        ...
        return {
          ...
          ...(acceptedEvidence?.storageId
            ? { storageId: acceptedEvidence.storageId }
            : {}),
      ...
      if (!hasCitizenContent)
        throw new Error("A citizen transcript entry has no supported content.");
      return {
        ...
        content: {
          ...
          ...(payload.voiceTranscript === undefined
            ? {}
    ...
    if (payload.source === "input_request") {
      if (
        ...
        !hasText(payload.inputRequest.prompt) ||
        payload.inputRequest.options.length === 0
      ) {
    ...
    if (payload.inputRequest)
      throw new Error("An Effi message has unexpected input-request metadata.");
    return {
      sourceMessageId: message.providerMessageId,
      sequence: message.sequence,
      direction: message.direction,
...
const validateTranscriptOrder = (
  messages: readonly Doc<"anonymousReportMessages">[],
): void => {
  if (messages.length === 0)
    throw new Error("A pending submission requires a persisted transcript.");
  if (messages.length > maxTranscriptMessages)
    throw new Error("The report transcript is too long to submit.");
  for (let index = 0; index < messages.length; index += 1) {
    if (messages[index]?.sequence !== index)
      throw new Error("The report transcript sequence is invalid.");
  }
};
...
const validateBrief = (
  ...
  if (!hasText(brief.summary) || brief.summary.length > 280)
    throw new Error("The case summary is invalid.");
  if (brief.category !== category)
    throw new Error(
      "The case brief category must match the confirmed category.",
    );
  if (
    brief.priority.reasons.length === 0 ||
    brief.priority.reasons.length > 5 ||
    brief.priority.reasons.some((reason) => !hasText(reason))
  ) {
    throw new Error("The priority recommendation requires valid reasons.");
  }
  if (brief.citations.length === 0 || brief.citations.length > 10)
    throw new Error("The case brief requires source citations.");
  for (const citation of brief.citations) {
    if (!hasText(citation.explanation))
      throw new Error("Every case citation requires an explanation.");
    if (
      citation.kind === "transcript_message" &&
      !transcriptMessageIds.has(citation.sourceMessageId)
    ) {
      throw new Error(
        "The case brief cites a transcript message outside this report.",
      );
    }
    if (
      citation.kind === "accepted_evidence" &&
      !acceptedEvidenceIds.has(citation.attachmentId)
    ) {
...
export const createPendingSubmission = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const draft = (
      await ctx.db
        .query("anonymousReportDrafts")
        .withIndex("by_scope_key_and_last_activity_at", (q) =>
    ...
    if (!draft || draft.phase === "cancelled" || draft.phase === "registered") {
      throw new Error(
        "No active anonymous draft exists for this pending submission.",
      );
    }
    if (draft.channel !== args.channel)
      throw new Error(
        "The pending submission channel does not match its draft.",
      );
    ...
    if (existing)
      return {
        pendingSubmissionId: existing._id,
        expiresAt: existing.expiresAt,
      };
    const messages = await ctx.db
      .query("anonymousReportMessages")
      .withIndex("by_draft_id_and_sequence", (q) => q.eq("draftId", draft._id))
      .order("asc")
      .take(maxTranscriptMessages + 1);
    validateTranscriptOrder(messages);
    const firstCitizenMessage = messages.find(
      (message) => message.direction === "citizen",
    );
    if (!firstCitizenMessage)
      throw new Error("The report transcript has no citizen message.");
    const transcriptMessageIds = new Set(
      messages.map((message) => message.providerMessageId),
    );
    if (transcriptMessageIds.size !== messages.length)
      throw new Error(
        "The report transcript contains duplicate source messages.",
      );
    for (const message of messages) {
      if (message.direction !== "citizen" || !("id" in message.payload))
        continue;
      if (
        message.payload.channel !== args.channel ||
        message.payload.conversationId !== args.conversationId
      ) {
        throw new Error(
          "A transcript message belongs to a different conversation.",
        );
      }
    }
    const locationIsPersisted = messages.some(
      (message) =>
        message.direction === "citizen" &&
    ...
    if (!locationIsPersisted)
      throw new Error(
        "The confirmed location is not present in the transcript.",
      );
    if (
      args.primaryEvidence.length === 0 ||
      args.primaryEvidence.length > maxAcceptedEvidence
    ) {
      throw new Error(
        "A pending submission requires between one and ten accepted evidence items.",
      );
    }
    const acceptedEvidenceIds = new Set(
      args.primaryEvidence.map((evidence) => evidence.attachmentId),
    );
    if (acceptedEvidenceIds.size !== args.primaryEvidence.length)
      throw new Error("Accepted evidence IDs must be unique.");
    for (const evidence of args.primaryEvidence) {
      if (
        !evidence.storageId ||
        (await ctx.storage.getUrl(evidence.storageId)) === null
      ) {
        throw new Error(
          "Accepted evidence must be uploaded to Convex Storage.",
        );
      }
      const sourceMessage = messages.find(
        (message) => message.providerMessageId === evidence.sourceMessageId,
      );
      if (
        !sourceMessage ||
        sourceMessage.direction !== "citizen" ||
        !("id" in sourceMessage.payload)
      ) {
        throw new Error(
          "Accepted evidence must reference a citizen transcript message.",
        );
      }
      const attachment = sourceMessage.payload.attachments?.find(
        (candidate) => candidate.id === evidence.attachmentId,
      );
      if (
        !attachment ||
        ...
        throw new Error(
          `Accepted evidence metadata does not match the inspected transcript attachment. Evidence: ${JSON.stringify(evidence)}. Stored: ${
            attachment
              ? JSON.stringify({
                  id: attachment.id,
                  kind: attachment.kind,
                  inspected: attachment.inspected,
    ...
    normalizedTranscript(
      messages,
      new Map(
        args.primaryEvidence.map((evidence) => [
...
const appMediaAssessment = v.union(
  v.literal("satisfactory"),
  v.literal("insufficient"),
);

/** Gives the gateway temporary read access only after the per-media key is verified. */
export const appReportMediaForAssessment = query({
  args: {
    serviceSecret,
  ...
  handler: async (ctx, args) => {
    ...
    if (
      media.assessmentKeyHash !== (await claimTokenHash(args.assessmentKey))
    ) {
...
/** Persists the model's verdict against exactly one authorized app upload. */
export const assessAppReportMedia = mutation({
  args: {
    serviceSecret,
    mediaId: v.id("appReportMedia"),
    assessmentKey: v.string(),
    assessment: appMediaAssessment,
  },
  returns: v.object({
    mediaId: v.id("appReportMedia"),
    assessment: appMediaAssessment,
  ...
  handler: async (ctx, args) => {
    ...
    return { mediaId: media._id, assessment: args.assessment };
  },
});
...
export const claimAuthenticatedSubmission = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (pending.claimedReportId) {
      ...
      if (!report)
        throw new Error("This registration link cannot be completed.");
      return {
    ...
    if (!pending.draftId)
      throw new Error(
        "This registration link does not contain a dashboard-ready report.",
      );
    const draftId = pending.draftId;
    const acceptedEvidence = pending.primaryEvidence.map((evidence) => ({
      ...evidence,
    }));
    const messages = await ctx.db
      .query("anonymousReportMessages")
    ...
    const acceptedEvidenceById = new Map(
      acceptedEvidence.map((evidence) => [evidence.attachmentId, evidence]),
    );
    ...
    const existingCitizen = await ctx.db
      .query("identities")
    ...
    for (const message of transcript)
      await ctx.db.insert("caseTranscriptMessages", { caseId, ...message });
    await ctx.db.insert("submissionAuditEvents", {
      pendingSubmissionId: pending._id,
      reportId,
      kind: "claimed",
...
export const reserveChannelAcknowledgement = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const report = await ctx.db
      .query("reports")
      .withIndex("by_report_number", (q) =>
        q.eq("reportNumber", args.reportNumber),
      )
      .unique();
    if (
      !report ||
      report.channel !== args.channel ||
      report.conversationId !== args.conversationId
    )
      throw new Error(
        "Report acknowledgement does not match its originating conversation.",
      );
    const existing = await ctx.db
      .query("channelAcknowledgements")
    ...
    await ctx.db.insert("channelAcknowledgements", {
      reportId: report._id,
      channel: report.channel,
      conversationId: report.conversationId,
      state: "reserved",
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(
      ...
      args.delivered
        ...
        : { state: "failed", failedAt: Date.now() },
    );
    return null;
  },
});
...
export const resumeOrAppendInbound = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const reusable =
      latest &&
      !isAnonymousDraftExpired(latest.lastActivityAt, args.receivedAt) &&
    ...
    const draftId =
      reusable?._id ??
      (await ctx.db.insert("anonymousReportDrafts", {
        scopeKey: args.scopeKey,
        channel: args.channel,
        phase: "gathering",
    ...
    if (!draftBeforeAppend)
      throw new Error("Draft disappeared while appending a citizen message.");
    const existing = await ctx.db
      .query("anonymousReportMessages")
      .withIndex("by_draft_id_and_provider_message_id", (q) =>
        q
          .eq("draftId", draftId)
    ...
    return {
      ...
      draft: { phase: draft.phase, sessionId: draft.sessionId },
      messages,
    };
  },
});
...
export const appendEffiTranscriptMessage = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const existing = await ctx.db
      .query("anonymousReportMessages")
      .withIndex("by_draft_id_and_provider_message_id", (q) =>
        q.eq("draftId", draft._id).eq("providerMessageId", args.eventId),
      )
      .unique();
...
export const cancelActiveDraft = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!draft || draft.phase === "registered" || draft.phase === "cancelled")
      return false;
    await ctx.db.patch(draft._id, {
      phase: "cancelled",
      lastActivityAt: args.cancelledAt,
    });
    const pending = await ctx.db
      .query("pendingSubmissions")
      .withIndex("by_draft_id", (q) => q.eq("draftId", draft._id))
...
export const updatePhase = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(draft[0]._id, {
      phase: args.phase,
...
export const syncDraftState = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (
      ...
      draft.phase === "registered"
    ) {
      throw new Error(
        "The durable draft is no longer active for this conversation.",
      );
    }
    await ctx.db.patch(draft._id, {
      phase: args.phase,
      ...
      const stored = await ctx.db
        ...
        .withIndex("by_draft_id_and_provider_message_id", (q) =>
          q
            ...
            .eq("providerMessageId", message.providerMessageId),
        )
        .unique();
...
const mediaKeysInPayload = (payload: unknown): string[] => {
  ...
  const voiceKey =
    typeof message.voice === "object" && message.voice !== null
      ? (message.voice as { storageKey?: unknown }).storageKey
      : undefined;
  return typeof voiceKey === "string"
    ? [...attachmentKeys, voiceKey]
    : attachmentKeys;
};
...
export const prepareExpiredAnonymousDraftErasure = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    const candidates = await Promise.all(
      expirablePhases.map(
        async (draftPhase) =>
          await ctx.db
            .query("anonymousReportDrafts")
            .withIndex("by_phase_and_last_activity_at", (q) =>
              q.eq("phase", draftPhase).lt("lastActivityAt", expiresBefore),
            )
            .take(1),
      ),
    );
    ...
    return {
      ...
      storageKeys: [
        ...new Set(
          batch.flatMap((message) => mediaKeysInPayload(message.payload)),
        ),
      ],
      isFinalBatch,
...
export const finalizeExpiredAnonymousDraftErasure = internalMutation({
  args: {
    ...
    messageIds: v.array(v.id("anonymousReportMessages")),
...
export const eraseExpiredAnonymousDrafts = internalAction({
  ...
  handler: async (ctx) => {
    const batch = await ctx.runMutation(
      internal.reporting!.prepareExpiredAnonymousDraftErasure!,
      {},
    );
    ...
    const response = await fetch(env.EFFI_GATEWAY_MEDIA_ERASURE_URL, {
      ...
      headers: {
        ...
        "x-effi-media-erasure-secret": env.EFFI_GATEWAY_CONVEX_SECRET,
      },
      body: JSON.stringify({ storageKeys: batch.storageKeys }),
    });
    if (!response.ok)
      throw new Error("The bot gateway did not erase anonymous draft media.");
    await ctx.runMutation(
      internal.reporting!.finalizeExpiredAnonymousDraftErasure!,
      batch,
    );
    await ctx.scheduler.runAfter(
      0,
      internal.reporting!.eraseExpiredAnonymousDrafts!,
      {},
    );
    return null;
  },
...
export const loadActive = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (
      !draft ||
      isAnonymousDraftExpired(draft.lastActivityAt, args.now) ||
      draft.phase === "registered" ||
      draft.phase === "cancelled"
    )
      return null;
    return {
      ...
      messages: await ctx.db
        .query("anonymousReportMessages")
        .withIndex("by_draft_id_and_sequence", (q) =>
          q.eq("draftId", draft._id),
        )
        .order("asc")
        .take(100),
