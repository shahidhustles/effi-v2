...
export default defineSchema({
  ...
  channelAcknowledgements: defineTable({ reportId: v.id("reports"), channel: v.union(v.literal("telegram"), v.literal("whatsapp")), conversationId: v.string(), state: v.union(v.literal("reserved"), v.literal("delivered"), v.literal("failed")), reservedAt: v.number(), deliveredAt: v.optional(v.number()), failedAt: v.optional(v.number()) }).index("by_report_id", ["reportId"])
});
