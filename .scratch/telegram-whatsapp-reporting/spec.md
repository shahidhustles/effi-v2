...
19. As a citizen, I want Effi to remember an incomplete complaint when I return to the same channel, so that I do not have to repeat earlier information.
...
27. As a citizen, I want an abandoned authentication attempt to preserve the pending submission, so that I can return and complete sign-in before it expires.
...
29. As a citizen, I want a new complaint to start with isolated memory, so that details from an older complaint do not leak into it.
...
- Allow one active report conversation per channel conversation. One report maps to one Eve session. After successful registration, a later complaint begins a fresh, memory-isolated session.
- Pass the complete persisted conversation to the model on each turn. Do not add per-turn structured draft output, compaction, summarized memory, embeddings, retrieval, or a separate structured-session memory layer in P0.
...
- Lock the report conversation after explicit confirmation. Messages received while authentication is pending receive a deterministic prompt to complete authentication and cannot steer the submission turn.
...
- The submission-preparation tool validates required fields, persists an immutable pending submission, and returns a short-lived, single-use authentication link bound to the pending submission and original channel conversation.
- Successful authentication creates exactly one report from the immutable pending submission and sends the report ID to the original channel. Post-confirmation edits are not supported; changing the report requires cancelling and returning to the confirmation flow.
...
- Run the reliability journey against both channel adapters: burst messages steer into one coherent response; duplicate events are ignored; interruption resumes the same report; accepted evidence survives provider failure; repeated authentication callbacks create one report.
...
- Verify authentication behavior: the link is short-lived, single-use, bound to the correct pending submission and channel conversation, and unsuccessful authentication does not create a report.
...
- Per-turn structured report drafts, conversation compaction, summarized memory, embeddings, semantic retrieval, or RAG
- Production use of Baileys or the contributor-tier model with real citizen data
- Advanced retention, archival, or automated expiration policies
